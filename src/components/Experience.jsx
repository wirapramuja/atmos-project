...
import { TextSection } from './TextSectoin';
...
We have a wide range of beverages!`
  ...
  useFrame((_state, delta) => {
    ...
    //FOLLOW THE CURVE POIIIIINT
    cameraGroup.current.position.lerp(curPoint, delta * 24);

    //MAE THE GROUP LOO AHEAD ON THE CURVE

    const looAtPoint = curve.getPoint(
      Math.min(lerpedScrollOffset + CURVE_AHEAD_CAMERA, 1)
    );

    const currentLooAt = cameraGroup.current.getWorldDirection(
      new THREE.Vector3()
    );

    const targetLookAt = new THREE.Vector3()
      .subVectors(curPoint, looAtPoint)
      .normalize();

    const lookAt = currentLooAt.lerp(targetLookAt, delta * 24);
    ...
    let anglleDegrees = (angle * 180) / Math.PI;
    anglleDegrees *= 2.4;

    // LIMIT PLANE ANGLE
    if (anglleDegrees < 0) {
      anglleDegrees = Math.max(anglleDegrees, -AIRPLANE_MAX_ANGLE);
    }
    if (anglleDegrees > 0) {
      anglleDegrees = Math.min(anglleDegrees, AIRPLANE_MAX_ANGLE);
    }

    //SET BACK ANGLE
    angle = (anglleDegrees * Math.PI) / 180;
